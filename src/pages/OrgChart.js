import { Alert, Drawer, Input, Popover, Tag, message } from "antd";
...
function OrgChart() {
    ...
    const [chartNotPossible, setChartNotPossible] = useState(false);
    ...
    useEffect(() => {
        if(state.currentEmployee){
            getDirectReportsByEmail(state.currentEmployee?.managerEmail, state.apiToken).then(res => {
                ...
                console.log(err);
                if(err.response.data == "Manager's profile not found"){
                    message.error("Manager's profile not found");
                    setChartNotPossible(true);
                }
    ...
    return ( <div id="orgchartpage">
                ...
                {
                    chartNotPossible ? <>
                    <Alert style={{margin: '0px 20px'}} showIcon message="Something went wrong! Organisational chart could not be built!" type="error" />
                    </>
                    :
                    <>
