import { Alert, Button, Table, Tabs, Tag, message } from "antd";
import { useAppContext } from "../context/AppContext";
import LeaveListCard from "../components/LeaveListCard";
import { BellOutlined, CheckCircleFilled, CheckCircleOutlined, CompassFilled, CompassOutlined, DownloadOutlined, FileDoneOutlined, WarningOutlined } from "@ant-design/icons";
...
function Requests() {
        ...
        setExpensesForTable(state.requestsForManager?.filter((application => application.type === "expense")).map(exp => (
            {
                ...
                actions:  <span>
                <Button onClick={() => {
                    updateExpense({id: exp.id, data: {status: 1}}, state.apiToken).then(() => {
                        window.location.reload();
                    }).catch((err) => {
                        message.error('Something went wrong');
                    })
                }} style={{color: successColor}}>Accept</Button>
                &nbsp;
                &nbsp;
                <Button
                onClick={() => {
                ...
                style={{color: errorColor}}>Reject</Button>
                </span>
            }
        )))
    ...
    return (
        <>
            <ToolBanner icon={<BellOutlined />} title={'Alerts'} subTitle={'Action items on you'} />
            <Tabs
                    label=""
                    id="requests-tabs"
                    defaultActiveKey="1"
